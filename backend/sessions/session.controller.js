...
export const register = async (req, res, next) => {
  ...
  try {
    const userExists = await dbGet('SELECT user_id FROM users WHERE email = ?', [email]);
    if (userExists) {
      return res.status(400).json({ success: false, message: 'Użytkownik o tym adresie e-mail już istnieje.' });
    }

    const salt = await bcrypt.genSalt(10);
    const hashedPassword = await bcrypt.hash(password, salt);

    const result = await dbRun('INSERT INTO users (name, email, password) VALUES (?, ?, ?)', [username, email, hashedPassword]);

    const token = jwt.sign({ id: result.lastID }, JWT_SECRET, { expiresIn: '1h' });
